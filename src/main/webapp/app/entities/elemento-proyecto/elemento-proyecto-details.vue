<template>
  <div class="row justify-content-center">
    <div class="col-8">
      <div v-if="elementoProyecto">
        <h2 class="jh-entity-heading" data-cy="elementoProyectoDetailsHeading">
          <span v-text="t$('ciecytApp.elementoProyecto.detail.title')"></span> {{ elementoProyecto.id }}
        </h2>
        <dl class="row jh-entity-details">
          <dt>
            <span v-text="t$('ciecytApp.elementoProyecto.descripcion')"></span>
          </dt>
          <dd>
            <span>{{ elementoProyecto.descripcion }}</span>
          </dd>
          <dt>
            <span v-text="t$('ciecytApp.elementoProyecto.elemento')"></span>
          </dt>
          <dd>
            <div v-if="elementoProyecto.elemento">
              <router-link :to="{ name: 'ElementoView', params: { elementoId: elementoProyecto.elemento.id } }">{{
                elementoProyecto.elemento.id
              }}</router-link>
            </div>
          </dd>

          <dt>
            <span>{{ t$('ciecytApp.elementoProyecto.dato') }}</span>
          </dt>
          <dd>
            <span v-html="elementoProyecto.dato"></span>
          </dd>
        </dl>
        <button type="submit" v-on:click.prevent="previousState()" class="btn btn-info" data-cy="entityDetailsBackButton">
          <font-awesome-icon icon="arrow-left"></font-awesome-icon>&nbsp;<span v-text="t$('entity.action.back')"></span>
        </button>
        <router-link
          v-if="elementoProyecto.id"
          :to="{ name: 'ElementoProyectoEdit', params: { elementoProyectoId: elementoProyecto.id } }"
          custom
          v-slot="{ navigate }"
        >
          <button @click="navigate" class="btn btn-primary">
            <font-awesome-icon icon="pencil-alt"></font-awesome-icon>&nbsp;<span v-text="t$('entity.action.edit')"></span>
          </button>
        </router-link>
      </div>
    </div>
  </div>
</template>

<script lang="ts" src="./elemento-proyecto-details.component.ts"></script>
